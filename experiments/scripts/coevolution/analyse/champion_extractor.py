...
def extract_champions_from_log(history_path: Path) -> Optional[Dict[str, Any]]:
        ...
        for line in f:
            try:
                ...
                if "probability" in event_data and "individual_id" in event_data:
                    latest_probs[str(event_data["individual_id"])] = float(event_data["probability"])
    ...
    # Sort survivors: primary key probability (desc), secondary key generation (asc)
    # Since we use reverse=True, we use -generation for ASC.
    survived_ids.sort(
        key=lambda iid: (
            latest_probs.get(iid, 0.0),
            -generation_born.get(iid, 999999) 
        ),
        reverse=True
    )
    ...
    for problem_dir in sorted(run_path.iterdir()):
        ...
        if history_file.exists():
            ...
            if champion_info:
                ...
                console.print(f"Extracted champion for {problem_dir.name}: {champion_info['id']} (p={champion_info['probability']:.4f}, gen={champion_info['generation']})")
            else:
                console.print(f"[yellow]Warning: No code champions found for {problem_dir.name}[/yellow]")
