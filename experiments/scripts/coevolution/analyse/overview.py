...
def analyze_matrix(df: pd.DataFrame, label: str) -> None:
    ...
    rprint(
        Text.assemble(
            ...
            (
                f" ({best_score}/{num_tests})",
                "green" if best_score == num_tests else "dim",
            ),
        )
    )
...
def main(
    run_ids: list[str] = typer.Option(
        ..., "--run-id", help="Run IDs to analyze (can be multiple)"
    ),
    ...
    problem_id: str = typer.Option(
        None, help="Specific Problem ID to analyze (optional)"
    ),
    ...
    for pid in sorted(all_problems.keys()):
                ...
                if not individuals_df.empty:
                    ...
                    if not code_data.empty:
                        ...
                        if valid_beliefs:
                            ...
                            champion_id_list = [
                                iid
                                for iid, b in valid_beliefs.items()
                                if b == max_belief
                            ]
        ...
        if len(completed_candidates) > 1:
            ...
            rprint(
                "[red]Please resolve this ambiguity by isolating the correct run.[/red]"
            )
        ...
        if sorted_by_belief and any(v > 0 for v in belief_map.values()):
            ...
            champions = [
                iid for iid in sorted_by_belief if belief_map[str(iid)] == max_belief
            ]
        ...
        summary_data.append(
            {
                ...
                "champion_passing": ", ".join(champion_passing_info)
    ...
    if incomplete_runs:
        ...
        rprint(
            ", ".join(
                [
                    f"[red]{r['problem_id']}[/red] [dim]({r['run_id']})[/dim]"
    ...
    stats_text = Text.assemble(
        ("Total Unique Problems: ", "bold"),
        (f"{total_problems}\n", "white"),
        ("Incomplete Items:     ", "bold"),
        (f"{total_incomplete}\n", "yellow"),
        ("Completed Items:      ", "bold"),
        (f"{total_completed}\n", "cyan"),
        ("Successful Solutions: ", "bold"),
        (f"{total_passed}\n", "green" if total_passed > 0 else "red"),
        ("Pass Rate (Completed):", "bold"),
