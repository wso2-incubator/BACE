...
def parse_coevolution_log(
    ...
    if target_problem_id:
        ...
        sanitized_run = sanitize_id(target_run_id)
        sanitized_pid = sanitize_id(target_problem_id)
        structured_path = (
            Path(log_dir) / sanitized_run / sanitized_pid / "evolutionary_history.jsonl"
        )
        if structured_path.exists():
            logger.info(
                f"Detected structured log format for {target_run_id}/{target_problem_id}"
            )
            return StructuredJSONLParser().parse(
                log_dir, target_run_id, target_problem_id
            )
    ...
    if not use_legacy or target_problem_id is None:
        ...
        sanitized_run = sanitize_id(target_run_id)
        run_path = Path(log_dir) / sanitized_run
