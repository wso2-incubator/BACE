...
def compose_property_test_script(
    snippet: str, input_arg: dict[str, Any], output: Any
) -> str:
    """Combine a property test snippet with one (input_arg, output) pair.
    ...
    Args:
        snippet:    Full property test function definition, e.g.::
                        ...
                        def property_sorted_output(input_arg, output):
                            return output == sorted(output)

        input_arg: Dictionary of function arguments.
        output:     Raw output value from the solution.
    ...
    safe_input = repr(input_arg)
    safe_output = repr(output)
    return (
        f"{snippet}\n\n"
        f"result = {fn_name}(\n"
        f"    input_arg={safe_input},\n"
